...
type ListCategoriesFn = Api<unknown>['categories']['v1ListCategories'];
...
type ListCategoriesAmountPerPeriodFn = Api<unknown>['categories']['v1ListCategoryAmountPerPeriod'];
...
export function getCategoriesQueryOpts(api: Api<unknown>, queryOpts?: QueryOpts) {
  return {
    ...
    queryFn: () =>
      api.categories
        .v1ListCategories(queryOpts as Parameters<ListCategoriesFn>[0])
        .then((res) => res.data),
  };
}
